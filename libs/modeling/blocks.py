...
class CFE(nn.Module):
    def __init__(
        ...
        self.low_pass_kernel_size = up_size

        self.low_pass = nn.Conv1d(n_embd, n_embd, up_size,stride=1, 
                                  padding=up_size // 2, groups=n_embd)
        self.pw_low = nn.Conv1d(n_embd, n_embd, 1, groups=n_embd)

        self.high_conv = nn.Conv1d(n_embd, n_embd, kernel_size, 
        ...
        if n_ds_stride > 1:
            if downsample_type == 'max':
                k_ds = n_ds_stride + 1
                s_ds = n_ds_stride
                p_ds = (n_ds_stride + 1) // 2
                self.downsample = nn.MaxPool1d(k_ds, stride=s_ds, padding=p_ds)
                self.stride = s_ds
    ...
    def reset_params(self, init_conv_vars=1):
        ...
        nn.init.constant_(self.low_pass.weight, 1.0 / self.low_pass_kernel_size)
        nn.init.constant_(self.low_pass.bias, 0.0)

        for conv in [self.pw_low, self.high_conv]:
            nn.init.normal_(conv.weight, mean=0.0, std=init_conv_vars)
    ...
    def forward(self, x, mask):
        ...
        F_low  = self.low_pass(out) * out_mask     # (B, C, T')
        F_high = (out - F_low) * out_mask           # (B, C, T')

        F_low_mod = self.pw_low(F_low)              # (B, C, T')

        energy      = torch.norm(F_high, p=2, dim=1, keepdim=True)  # (B,1,T')
        energy_gate = torch.sigmoid(self.energy_proj(energy))        # (B,1,T')
        F_high_proc = self.high_conv(F_high)
        F_high_mod  = F_high_proc * (1.0 + self.alpha * energy_gate)  # (B,C,T')

        pre_attn = (F_low_mod + F_high_mod + out)
        attn_out, attn_out_mask = self.attn(pre_attn, out_mask.bool())
        attn_out = (pre_attn + out + attn_out)
