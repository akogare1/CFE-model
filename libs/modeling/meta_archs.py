...
class CFE(nn.Module):
    ...
    def __init__(
            ...
            n_head,
            n_mhca_win_size,  # window size w for mhca
            ...
            k,  # the K in CFE
            init_conv_vars,  # initialization of gaussian variance for the weight in CFE
        ...
        if isinstance(n_mhca_win_size, int):
            self.mhca_win_size = [n_mhca_win_size] * len(self.fpn_strides)
        else:
            assert len(n_mhca_win_size) == len(self.fpn_strides)
            self.mhca_win_size = n_mhca_win_size
        max_div_factor = 1
        for l, (s, w) in enumerate(zip(self.fpn_strides, self.mhca_win_size)):
        ...
        assert backbone_type in ['CFEBackbone']
        ...
        self.backbone = make_backbone(
            ...
            **{
                ...
                'mhca_win_size': self.mhca_win_size,
